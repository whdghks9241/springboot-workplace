...
public class ProductController {
	...
	public String pageList(Model model, @RequestParam(value="page", defaultValue="0") int page) {
		...
		// value="page" 값으로 page 이름을 ㅏㄷ기로 지정
		// 만약에 초반이나 후반에 어떤 값이 page 안에 없다면 
		// page가 null 값이라면 기본 값으로 0으로 설정해서 초기값을 null이 아닌 0으로 처리하겠다
...
//	}
	...
	public String addComment(@RequestParam int productId, @RequestParam String commentContent) {
		...
		return "redirect:/product/detall/" + productId;
	}
	
	// like 한 내용 받아줄 수 있게 PostMapping
	public String likeProduct() {
//		productService.likeProduct(/* 추후 아이디 값이나 like를 넣어줄 수 있는 변수 값*/);
